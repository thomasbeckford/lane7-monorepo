import { getVenuesByCountryCode } from '@/server/getVenues';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const venues = await getVenuesByCountryCode('DE');
  ...
  const totalGames = venues.reduce((total, venue) => {
    if (venue.availableGames && Array.isArray(venue.availableGames)) {
      return total + venue.availableGames.length;
    }
    return total;
  }, 0);
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <Section background="default" spacing="xl" className="bg-gradient-to-br from-purple-900 via-black to-pink-900">
        <SectionHeader className="text-center">
          ...
          <SectionDescription className="text-xl md:text-2xl text-white/90 max-w-4xl mx-auto">
            Entdecke unsere erstaunlichen Venues in Deutschland.
          </SectionDescription>
        </SectionHeader>
      </Section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-white/60 mt-4 text-lg">
              {totalVenues} venue{totalVenues !== 1 ? 's' : ''} available
            </p>
          </div>
          ...
          {venues.length > 0 ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-white/60 text-lg">We're working on bringing amazing venues to Germany. Stay tuned!</p>
            </div>
          )}
        </div>
      </div>
      ...
      <Section spacing="lg" className="bg-white/5">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          ...
          <div>
            <div className="text-4xl md:text-5xl font-black text-yellow-400 mb-2">1</div>
            <div className="text-white/80 uppercase tracking-wide font-medium">COUNTRY</div>
          </div>
          <div>
            <div className="text-4xl md:text-5xl font-black text-yellow-400 mb-2">{totalGames || '50+'}</div>
